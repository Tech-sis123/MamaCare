...
exports.aiService = {
    ...
    async generatePersonalizedLesson(params) {
        ...
        if (!env_1.env.OPENAI_API_KEY || env_1.env.OPENAI_API_KEY === 'your_openai_api_key_here' || env_1.env.OPENAI_API_KEY === '') {
            logger_1.logger.info('OpenAI API Key is missing or default. Skipping AI personalization (graceful degradation).');
            return null;
        }
        try {
            logger_1.logger.info({ patientName, weekNumber, languagePreference }, 'Requesting personalized AI antenatal lesson from OpenAI...');
...
Keep the total output under 250 words. Format with standard paragraphs.`;
            ...
            logger_1.logger.error({ err }, 'Failed to generate personalized AI lesson');
            return null;
        }
    },
};
