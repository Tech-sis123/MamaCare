...
exports.detectDangerSigns = detectDangerSigns;
const HIGH_BP_SYSTOLIC = 140;
const HIGH_BP_DIASTOLIC = 90;
function hasHighBP(ctx) {
    return ((ctx.bp_systolic != null && ctx.bp_systolic >= HIGH_BP_SYSTOLIC) ||
        (ctx.bp_diastolic != null && ctx.bp_diastolic >= HIGH_BP_DIASTOLIC));
}
function hasSymptom(symptoms, key) {
    return symptoms.some((s) => s.symptom_key === key);
}
function hasSevereSymptom(symptoms, key) {
    return symptoms.some((s) => s.symptom_key === key && s.severity === 'severe');
}
function detectDangerSigns(symptoms, context) {
    const triggers = [];
    // 1. Severe headache + high BP
    ...
    // 5. Reduced fetal movement after 28 weeks
    if (hasSymptom(symptoms, 'reduced_fetal_movement') &&
    ...
    // 9. Sudden facial or hand swelling + headache
    if (hasSymptom(symptoms, 'facial_swelling') &&
